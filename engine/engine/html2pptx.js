...
async function addBackground(slideData, targetSlide, tmpDir) {
  if (slideData.background.type === 'image' && slideData.background.path) {
    let imagePath = decodeURIComponent(slideData.background.path.startsWith('file://')
...
function addElements(slideData, targetSlide, pres) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      let imagePath = decodeURIComponent(el.src.startsWith('file://') ? el.src.replace('file://', '') : el.src);
      ...
      if (el.tagName === 'TABLE') {
        ...
        trElements.forEach((tr, rowIdx) => {
          ...
          cells.forEach((cell, colIdx) => {
            ...
            // Get cell text with inline formatting
            const hasFormatting = cell.querySelector('b, i, u, strong, em, span, br');
            let cellText;

            if (hasFormatting) {
              const runs = parseInlineFormatting(cell, {});
              cellText = runs.length > 0 ? runs : cell.textContent.trim();
            } else {
              cellText = cell.textContent.trim();
            }

            // Parse border for this cell
            const borderWidthPt = pxToPoints(cellComputed.borderBottomWidth);
            const hasCellBorder = borderWidthPt > 0;

            rowData.push({
              text: cellText,
              options: {
                fill: cellComputed.backgroundColor !== 'rgba(0, 0, 0, 0)' 
                ...
                bold: cellComputed.fontWeight === 'bold' || parseInt(cellComputed.fontWeight) >= 600,
                align: cellComputed.textAlign === 'start' ? 'left' : cellComputed.textAlign,
                valign: cellComputed.verticalAlign === 'middle' ? 'middle' : 
                        cellComputed.verticalAlign === 'bottom' ? 'bottom' : 'top',
                margin: [
        ...
        // Calculate table border from CSS
        const tableBorderWidth = pxToPoints(tableComputed.borderWidth);
        const tableBorderColor = rgbToHex(tableComputed.borderColor);

        // Check for border-collapse
        const borderCollapse = tableComputed.borderCollapse === 'collapse';
